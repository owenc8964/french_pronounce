...
> ⭐ **2026-08-28 補：六個階段之後還有一層「升級」。**
> 階段 6 走完 ＝ 這座島在**當前等級**完成了，不等於它到頂。之後每學到一個 B1/B2 結構
> （subjonctif／conditionnel／讓步連接詞），要**回頭往這座島加房間**——加讓步段與假設段，
> ⛔ 不是把原本的句子換成難字。材料就用該島 `follow_ups` 裡 `cover:'gap'` 的追問。
> **時機跟著課程走，不排在考前**；⚠️ 升級主要升**寫版**，說版只加內容深度、句法維持壓縮。
> 完整說明見 [`STRATEGY_PRODUCTION_9M.md`](STRATEGY_PRODUCTION_9M.md) 原則 6。
