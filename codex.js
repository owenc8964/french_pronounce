...
{ n:'1', name:'名詞與限定詞', fr:'Le nom et ses déterminants', sections:[
 ...
 { n:'1-1', name:'名詞的性別', fr:'Le genre', items:[
  { n:'1-1-1', lvl:'A1', name:'陰陽性的本質', fr:'masculin / féminin',
    ...
    pts:['拉丁文三性崩塌成兩性的遺產：性別跟東西本身無關（la table 桌子沒有女性特質）','背名詞永遠連冠詞一起背：不背 livre，背 le livre','形容詞、冠詞、所有格、分詞全部看它穿制服——性別是全句配合的源頭'],
    ex:[{fr:'le livre / la table', zh:'書（陽）／桌子（陰）'}],
  ...
  { n:'1-1-2', lvl:'A2', name:'字尾判性訣竅', fr:'terminaisons',
    ...
    pts:['偏陰性：-tion/-sion（la nation）、-té（la santé）、-ure、-ette、-ance/-ence、-ie','偏陽性：-age（le fromage）、-ment、-eau、-isme、-oir','-e 結尾「傾向」陰性但例外極多，只當弱線索'],
    ex:[{fr:'la nation, la santé, le fromage, le gouvernement', zh:'依字尾判性'}],
    exc:['-age 例外：la page、la plage、l\'image（陰）','le musée、le lycée（-ée 卻陽性）','l\'eau（陰）'] },
  { n:'1-1-3', lvl:'A1', name:'人與職業的陰陽形', fr:'noms de personnes',
    brief:'指人的名詞跟著人的性別變形',
    pts:['+e：étudiant→étudiante；-eur→-euse（vendeuse）；-teur→-trice（actrice）；-ien→-ienne（musicienne）','部分職業歷史上只有陽性形，現代法語正在補陰性形（la professeure）'],
    ex:[{fr:'un étudiant / une étudiante', zh:'男學生／女學生'},{fr:'un acteur / une actrice', zh:'男演員／女演員'}] },
 ]},

 { n:'1-2', name:'名詞的複數', fr:'Le nombre', items:[
  { n:'1-2-1', lvl:'A1', name:'規則複數 +s', fr:'pluriel en -s',
    brief:'加 s 但不發音——複數靠冠詞聽出來',
    pts:['書寫 +s，口語完全聽不到：le livre / les livres 差在 le→les','所以冠詞才是法文複數的真正載體'],
    ex:[{fr:'les livres, les tables', zh:'書（複）、桌（複）'}] },
  { n:'1-2-2', lvl:'A2', name:'-eau/-eu→+x、-al→-aux', fr:'pluriels en -x',
    brief:'兩個古法語拼寫家族',
    pts:['-eau/-eu 加 x：les gâteaux、les cheveux','-al 變 -aux：le journal → les journaux、l\'animal → les animaux'],
    ex:[{fr:'des gâteaux, des journaux', zh:'蛋糕們、報紙們'}],
 ...
 { n:'1-3', name:'冠詞', fr:'Les articles', topics:['articles'], items:[
  { n:'1-3-1', lvl:'A1', name:'定冠詞', fr:'le / la / l\' / les',
    ...
    pts:['le（陽）la（陰）l\'（母音前）les（複數）','法文泛指整類也用定冠詞：J\'aime le café（我喜歡咖啡這種東西）——中文英文都不加，這是坑'],
    ex:[{fr:"J'aime la musique.", zh:'我喜歡音樂。（泛指）'}], gram:'articles' },
  { n:'1-3-2', lvl:'A1', name:'不定冠詞', fr:'un / une / des',
    brief:'第一次提到、隨便哪一個',
    pts:['un（陽）une（陰）des（複數）','英文的 a/an ＋ 一個英文沒有的複數版 des'],
    ex:[{fr:'Il y a un chien et des chats.', zh:'有一隻狗和一些貓。'}] },
  { n:'1-3-3', lvl:'A2', name:'部分冠詞', fr:'du / de la / de l\'',
    brief:'不可數的「一些」——吃喝的預設冠詞',
    pts:['字面＝de+定冠詞＝「那東西的一部分」：Je mange du pain（吃一些麵包，不是整條）','抽象名詞也用：avoir du courage、faire du sport'],
    ex:[{fr:'Je bois de l\'eau et du café.', zh:'我喝水和咖啡。'}],
    vs:[{n:'1-3-4', note:'du 有兩個身分：部分冠詞 vs de+le 縮合，長一樣意思不同'}] },
  { n:'1-3-4', lvl:'A1', name:'縮合冠詞', fr:'du / au / des / aux',
    brief:'de+le、à+le 黏成一個音的化石',
    pts:['de+le=du、de+les=des、à+le=au、à+les=aux','de la / à la / de l\' / à l\' 不縮合（只有 le/les 被吃掉）'],
    ex:[{fr:'Je vais au marché, je parle du prof.', zh:'我去市場、我談到老師。'}] },
  { n:'1-3-5', lvl:'A2', name:'變 de 的場合', fr:'la règle du « de »',
    brief:'否定與量詞之後，冠詞塌縮成 de',
    pts:['否定後：du pain → pas de pain（數量歸零）','量詞後：beaucoup de、un peu de、trop de、un kilo de','形容詞放複數名詞前：des → de（de beaux objets）'],
    ex:[{fr:'Je ne mange pas de viande.', zh:'我不吃肉。'},{fr:'beaucoup de pâtes', zh:'很多麵'}],
    exc:['être 的否定不變：Ce n\'est pas du café（身分判斷句保留原冠詞）'] },
  { n:'1-3-6', lvl:'A2', name:'不加冠詞的場合', fr:'l\'absence d\'article',
    brief:'少數位置冠詞消失',
    pts:['職業當屬性：Il est professeur（c\'est un professeur 才有冠詞）','城市與多數國名搭 de：Je viens de France / de Taïwan','月份、星期直接裸用：en janvier、lundi'],
    ex:[{fr:'Elle est médecin.', zh:'她是醫生。（無冠詞）'}],
 ...
 { n:'1-4', name:'指示詞', fr:'Les démonstratifs', items:[
  { n:'1-4-1', lvl:'A1', name:'指示形容詞', fr:'ce / cet / cette / ces',
    brief:'「這個/那個」，法文不分遠近',
    pts:['ce（陽）cet（陽+母音）cette（陰）ces（複數）','cet hôtel 的 cet＝母音相撞禁忌（跟 mon amie 同理）','要強調遠近才加 -ci/-là：ce livre-ci / ce livre-là'],
    ex:[{fr:'cet ordinateur, cette valise, ces téléphones', zh:'這台電腦、這個行李箱、這些手機'}],
    topics:['demonstrative-adj'], gram:'determinants' },
  { n:'1-4-2', lvl:'B1', name:'指示代名詞', fr:'celui / celle / ceux / celles',
    brief:'「那個～的」——把名詞收掉再指一次',
    pts:['celui de Paul（保羅的那個）、celle qui parle（正在說話的那位）','後面必接 de、-ci/-là 或關係子句，不能單獨站'],
 ...
 { n:'1-5', name:'所有格', fr:'Les possessifs', items:[
  { n:'1-5-1', lvl:'A1', name:'所有格形容詞', fr:'mon / ma / mes …',
    ...
    pts:['mon/ma/mes、ton/ta/tes、son/sa/ses、notre/nos、votre/vos、leur/leurs','⚠️ sa femme＝他或她的太太——看 femme 陰性，不看主人（英文 his/her 的相反邏輯）'],
    ex:[{fr:'Sa femme est fleuriste.', zh:'他的太太是花店老闆。'}],
    topics:['family-possessives'], gram:'determinants' },
  { n:'1-5-2', lvl:'A1', name:'母音前借陽性形', fr:'mon amie',
    brief:'ma amie 母音相撞，借 mon 來墊',
    pts:['陰性名詞母音開頭 → ma/ta/sa 改 mon/ton/son','純發音規則，amie 還是陰性（mon amie est belle）'],
  ...
  { n:'1-5-3', lvl:'B1', name:'所有格代名詞', fr:'le mien / la tienne …',
    brief:'「我的那個」——名詞收掉的所有格',
    pts:['le mien/la mienne/les miens、le tien、le sien、le nôtre、le vôtre、le leur','冠詞+性數都要配合被代替的名詞'],
 ...
 { n:'1-6', name:'數與量', fr:'Les nombres et quantités', items:[
  { n:'1-6-1', lvl:'A2', name:'量詞', fr:'expressions de quantité',
    brief:'量詞後面一律 de + 裸名詞',
    pts:['beaucoup de、un peu de、assez de、trop de、plus de','單位：un kilo de、une bouteille de、une douzaine de'],
    ex:[{fr:'un kilo de tomates, beaucoup de travail', zh:'一公斤番茄、很多工作'}],
    vs:[{n:'1-3-5', note:'就是「變de」規則的量詞分支'}] },
  { n:'1-6-2', lvl:'A1', name:'數字系統', fr:'les nombres',
    brief:'70/80/90 是古代二十進位的化石',
    pts:['soixante-dix（60+10）、quatre-vingts（4×20）、quatre-vingt-dix（4×20+10）','21/31…用 et un：vingt et un','比利時/瑞士說 septante、nonante——證明這是歷史包袱不是邏輯'],
    ex:[{fr:'quatre-vingt-quinze = 95', zh:'4×20+15'}],
    exc:['quatre-vingts 有 s，但 quatre-vingt-un 後面接數字就沒 s'] },
  { n:'1-6-3', lvl:'A2', name:'序數', fr:'les ordinaux',
    brief:'字尾 -ième，只有「第一」特殊',
    pts:['deuxième、troisième…；premier/première 是唯一完全不規則','日期只有1號用序數：le premier mai，其他用基數（le 4 juin）'],
    ex:[{fr:'le premier étage, la deuxième rue', zh:'二樓（法式一樓）、第二條街'}] },
 ]},
]},
