...
### ⭐ 2026-09-22 Owen 下修每日上限（現行值，取代下面的舊值）

Owen：「anki 的難度比我想像中高，當累積到一定的量的時候很難記得，也壓力很大」
「**我先下修到一天最多 60 題複習、6 題新題目**」「至少讓我願意繼續、不會堆得到處都是是最重要的」
「由於你內部還會有例句我其實還會去背誦那個…**大概 30 張我就頭很痛了**」

| 牌組 | 新卡／天 | 複習／天 |
|---|---|---|
| `Français::Cours` | **6** | **60** |
| `Français::Oral` | **0**（25 張先停著，等 Cours 穩了再開） | **60** |
| 父牌組 `Français` | **6** | **60** |

⚠️ **三層都要設**，因為上限掛在組態(preset)上，而**父牌組的上限只有從父牌組點進去學才生效**
→ ⭐ **以後每天從父牌組 `Français` 進去**，不要直接點子牌組（不然兩個子牌組各 6 會變成 12）。

⚠️ **牌組名稱**：實際是 `Français::Cours` 與 `Français::Oral`（不是舊文件寫的 `Mining`；
組態清單裡仍留著一個叫 `Mining` 的 preset）。

### ⚠️⚠️ 09-22 診斷：壓力的根因不是「量」，是「每一張都是最重的那種」

用 `tools/anki_precheck.py` 同款的唯讀方式查收藏檔，事實如下：

- **積欠 0、今天到期 0** → ⭐ **Owen 根本沒落後**，他是跟得上的
- 總數 711（Cours 686＋Oral 25），其中 **561 張是還沒學過的新卡**
- 一張 note ＝ 一張卡（不是雙向）
- ⭐⭐ **643 張卡全部是 `fr::tier::production`（看中文→產出法文），recognition 0 張**

→ ⛔ 這違反 memory `feedback_production_vs_recognition_load`（「別整批都出 Production 卡；
純生成批次每日上限降到 5」）。生成卡最耗工作記憶，而牌組是 100% 生成卡，
再加上卡裡的例句與文法註解 Owen 會一起背 → 每張的實際負擔再乘上去。
**「30 張就頭很痛」是配比問題，不是他撐不住。**
⏸ **待 Owen 拍板**：要不要把一部分改成 recognition 方向（看法文→中文，輕得多），
讓每天的量裡有輕有重，而不是只調數量。

<details><summary>舊值（2026-08-17～2026-09-22）</summary>

- `Français::Cours`：新卡上限 10／天、複習上限 不限（用預設組態）
- `Français::Mining`：新卡上限 5／天、複習上限 不限
- 當時的理由：新卡上限沿用原系統的數字（複習卡 10／造句 5），節奏不會突然改變。
...
### 撇號存成 `&#x27;`（2026-08-29 發現，**結論是不處理**）

**現象**：`FR_COURS_REC_001–023` 裡有 7 張、共 13 處，資料庫存的是 `l&#x27;avion` 而不是 `l'avion`。
後面幾批都沒有這個現象。

**影響有多大**：
- 卡片畫面**完全正常**（Anki 把欄位當 HTML 算繪，`&#x27;` 就顯示成 `'`）
- 唯一的實際差別：**Anki 內搜尋 `l'avion` 找不到那張卡**

**為什麼不修**：能想到的修法都要靠「匯入時勾允許 HTML」，
而 2026-08-19 的結論就是**把這個勾選框變成無關緊要**（TSV 不寫標籤 → 勾不勾都正常）。
為了一個只影響搜尋的小瑕疵，把工作流程重新綁回一個勾選框，划不來。
⚠️ **成因也還沒實測確認**（只知道第一批有、後面幾批沒有），別當成已知結論拿去改流程。

→ **維持原則不變：TSV 一律不寫 HTML 標籤（強調用全形括號），匯入時勾不勾都可以。**

---

### 檔案擁有者對照（2026-08-29 補齊，每個 ID 都要有家）

> 鐵律：**一個 ExternalID 只能被一個檔案擁有**。要修一張舊卡，就改「原本那個檔案」裡的那一列，
> ⛔ 不要另開修復檔——兩個檔案同時定義一個 ID，就是 2026-08-19 弄丟 3 張卡的那個結構。

| 檔案 | ID 範圍 | 張數 | 內容 |
|---|---|---|---|
| `anki/anki_batch01_recycled.tsv` | `REC_001–023` | 23 | 第一批回鍋字（2026-08-29 從作廢檔搬出來，原本沒有現役擁有者）|
| `anki/anki_batch02_recycled.tsv` | `REC_024–066` | 43 | 第二批回鍋字（老師「看過了」的字）|
| `anki/anki_repair_lost3.tsv` | `REC_067–069` | 3 | 08-19 弄丟後補回的三張 |
| `anki/anki_l26.tsv` | `REC_070–097` | 28 | 第26課 |
| `anki/anki_l27_l28.tsv` | `REC_098–129` | 32 | 第27、28課 |

⚠️ `anki/_已匯入作廢/anki_recycled_v2.tsv.已作廢-請勿匯入` 是歷史檔，**不要再匯入**——
它同時涵蓋 001–059，跟上面兩個現役檔重疊。
...
### 一個 ID 只能有一個檔案擁有它（2026-08-23 立規）

要修一張已經匯入的卡，**就去改「原本那個檔案」裡的那一列**——不要另外開一個修復檔。兩個檔案同時定義同一個 ExternalID，就是 08-19 弄丟 3 張卡的那個結構：之後任何一次重匯都可能用舊版蓋掉新版，而且沒有人知道哪一份才算數。

- 新一批卡 → 新檔案，號碼從收藏檔最大號 +1 開始。
- 修舊卡 → 改舊檔案的那一列，重匯整個檔（其餘列會是「純更新」，不痛不癢）。
- `anki_precheck.py` 一次餵多個檔會自動檢查有沒有 ID 重疊。
...
**待匯入的檔案放桌面**（2026-08-23 Owen：「丟桌面吧！我比較好找」），**匯完就收回專案的 `anki/`**（已 gitignore，只留本機）。

⚠️ **關鍵是「只留一份」**：用移動不是複製。桌面留一份、`anki/` 又留一份的話，兩份會慢慢長歪，之後誰也不知道該匯哪一份——那跟「一個 ID 兩個擁有者」是同一個病。

- 待匯入 → `~/Desktop/`
- 匯完 → 移回 `anki/`（真相來源其實是 Anki 收藏檔本身，有 AnkiWeb 同步當備份；TSV 只是工作區）
- 已作廢、跟收藏檔衝突的 → `anki/_已匯入作廢/`，**絕對不要留在桌面**——一個誤點就覆蓋掉一批卡
