...
1. **真實教材來源**：要 Owen 背誦、複習、練習的內容（quiz 題目、複習卡、句庫、造句題）必須出自 `french_notes.html` 或 Owen 提供的課程材料，**禁止 Claude 自創法文句子當教材**。已核准的例外：聽力/閱讀「理解測驗」的題幹與短文可原創，但詞彙句型必須對齊已學課次
   - **來源三層優先序**（2026-08-26 Owen 拍板）：**標準法文正確性 ＞ 課本 ＞ 逐字稿**。逐字稿是語音辨識產物、課本是商業教材，**兩者都是線索不是判準**，課本也會有錯（第26課老師自己就否定了課本把 `sec = mince` 畫等號）
   - **入庫前一定先對課本**：`assets/.textbook_cache.txt` 找到對應頁，Fonctionnement 表、Entraînement 題目與答案逐條對照，逐字稿糊掉的地方以課本為準
   - **衝突時以標準法文寫，但要把課本／老師的說法標出來**，不要沉默改掉——Owen 手上就是那本課本，看到不一致會困惑。老師的口語簡化 ≠ 規則（案例：第27課老師說 `notre` 與 `le nôtre`「發音一樣」，標準法文是開口 [ɔ] vs 閉口 [o]，那是口語中和現象）
