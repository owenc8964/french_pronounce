...
## 練習頁鐵律：⭐ 跳過必須是合法動作（2026-09-11 Owen 指定，適用所有現有與新增頁面）

> Owen 放棄了一個每天的練習，原因是：
> 「我幾乎沒辦法填完，非常挫折。**很多問題我其實想跳過的，我已經覺得會了、可以了，
>  但我沒有填完我就無法完成，那我乾脆不完成了**」
> 「**不是今日完成而已，是每一頁中有些題目我不想填了想跳過，但沒辦法**」
> 「**應該要有一個一鍵跳過此題的選項**」「**其他頁面也改吧！之後新增頁面也都要用這條**」

⚠️⚠️ **這件事本來就違反專案自己的鐵律**——教學鐵律明寫 ⛔「不要為了『完整』讓他做已經反射化的題目」
（重講已經會的「只是浪費，且讓他覺得自己很笨」），`feedback_fun_is_the_engine` 也寫 ⛔「系統不可以罵他」。
⭐ **是系統在違反自己的規則，不是他偷懶。**

### 任何有「作答 → 判定完成」的頁面，必須同時提供三層出口
| 層 | 動作 | 語意 |
|---|---|---|
| **格／題** | 「會了」 | 這一格我會了，這一輪不算 |
| ⭐ **整題／整頁** | 「⏭ 這題跳過」 | 一次跳掉這頁**還沒填的**格子。⚠️ **已經填了字的不動**——那是他真的想作答的 |
| **今日步驟** | 「今天跳過」 | 整個步驟今天不做，⭐ **且不擋住後面的步驟** |

### ⛔ 跳過的四條硬規則
1. **不計分**：分母要扣掉跳過的（7 格跳 2 → 顯示 `1/5`，⛔ 不是 `1/7`）
2. **不進複習輪 / 不記錯題本**：跳過 ≠ 答錯
3. ⭐ **重來時保持跳過**：整張重考時跳過的格要維持跳過，⛔ 不然他又被擋住（實測踩過）
4. **措辭中性**：badge 用灰色，⛔ 不是紅的（不是失敗）也不是綠的（不是完成）。
   ⛔ 不准出現「你跳過了 N 題」這種帶罪惡感的句子

### ⚠️ 但跳過要記錄，⛔ 不可永久移除
記進 `clb7_*_known`（物件形狀 ＋ `last` 欄位，配合 `sync_supabase.js` 的物件合併慣例）。
⚠️ **理由**：自我評估的準確度很低（`research/2026-09-06_SOLO_SYNTHESIS`：自評 vs 真實 **r ≈ .46**，
口說只有 .18–.55）——⭐ **「我覺得我會了」不能當成事實**。
→ 記次數可以拿來**降低出現頻率**，⛔ 但不可以把那格刪掉。

⭐ **黃金範本：`table_drill.html`**（2026-09-11 實作，三層都有）。新頁面照它做。
